...
タイマー(F2キー)との同期精度を上げるため、「やめ」の瞬間に書き込み中
だったセグメントも切り出し対象に含める。ただし、書き込みの途中で読みに
行くと、書きかけの不完全なNALユニットを読んでしまいH.264として破損した
映像になることがある(ラウンド5の実機検証で確認、実質ほぼ毎回発生する)。

そこでラウンド6では、「やめ」の瞬間に書き込み中だったセグメントを特定し、
それが完全に書き終わる(=次のセグメントに切り替わる)のを待ってから
読みに行く方式にした。待ち時間は最大でもSEGMENT_SECONDS秒程度で済み、
同期精度は落とさずに読み取り競合を回避できる(孝さんの発案による設計)。
待ってもなお破損していた場合の保険として、生成直後にデコード検証も行い、
それでも壊れていたら最新セグメントを除外した安全な方式で作り直す。

この待ち時間があるため、extract_on_yame() は呼び出し元をブロックする
(最大でSEGMENT_SECONDS+1秒程度)。物理ボタン(--input-mode button)経由の
場合、pynputのキーボードフックのコールバック内でこの待ちをそのまま行うと、
Windowsがフックを応答なしとみなして無効化してしまう危険があるため、
main.py側で「やめ」処理を別スレッドに逃がして呼び出すこと。
"""
...
class ClipExtractor:
    def __init__(self, running_counter_fn=None):
        ...
        # あると一時的にそれを超えることがある。
        # 各要素: {"path": str, "created_counter": float or None, "match": int}
        self._slots = []
        # 「試合」番号。1試合の中で複数回「やめ」が起きても同じ番号がつき、
        # 試合後にまとめてレビューする際にグループ表示できるようにする。
        # 現状は記録員が手動で進める(自動採番)想定だが、将来的に空手タイマー
        # システム側が保持している試合番号を読み取って設定する形に差し替え
        # られるよう、変更経路をこの1メソッド(set_match/next_match)に
        # まとめてある。
        self._current_match = 1

    def next_match(self):
        """記録員が「次の試合へ」を押した時に呼ぶ。現在の試合番号を1つ進める"""
        self._current_match += 1
        return self._current_match

    def set_match(self, match_number):
        """
        試合番号を明示的に設定する。将来、空手タイマーシステム側が持つ
        試合番号を読み取って同期する場合はこちらを使う想定。
        """
        self._current_match = match_number
        return self._current_match
    ...
    def extract_on_yame(self):
        ...
        落とさずに読み取り競合を避ける。それでも壊れていた場合の保険として、
        デコード検証→最新セグメント除外での作り直しにフォールバックする。
        """
    ...
    def list_current_clips(self):
        """監査画面に表示する現在保持中のクリップ一覧(古い順、試合番号付き)"""
        return [{"path": s["path"], "match": s["match"]} for s in self._slots]

    def get_match_for_clip(self, filename):
        """保存(save_clip)時に、そのクリップがどの試合のものだったかを引くために使う"""
        basename = os.path.basename(filename)
        for s in self._slots:
            if os.path.basename(s["path"]) == basename:
                return s["match"]
        return None

    def clear_all(self):
        """
        現在保持中のライブクリップをすべて削除する(一括削除ボタン用)。
        data/saved/ に保存済みのコピーはこの操作では一切削除しない
        (保存は別ファイルへのコピーのため、影響を受けない)。
        """
