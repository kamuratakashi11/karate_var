...
class TimerSyncedKeyListener:
    ...
    def _fire(self):
        # extract_on_yame() はラウンド6の変更で、セグメントの書き終わりを
        # 待つため最大数秒ブロックしうる。ここ(pynputのキーボードフックの
        # コールバックスレッド)で直接ブロックすると、Windowsがフックを
...
class NextMatchKeyListener:
    """
    「次の試合へ」を進めるためのグローバルホットキー監視。

    F2トグル判定(TimerSyncedKeyListener)とはタイマー状態を共有せず完全に
    独立している。--input-mode enter/button のどちらで起動していても常に
    有効にする想定のため、あえて別クラスにしてある(タイマー同期の有無に
    関係なく、記録員がPCのキーボードから直接「次の試合へ」を押せるように
    するための機能のため)。
    """

    def __init__(self, on_next_match):
    ...
    def handle_key_event(self, key_name):
        ...
        pynputに依存しないロジック部分だけを切り出してあるので、
        テスト時はpynput無しでもこのメソッドを直接呼んで検証できる。
        戻り値: 実際にon_next_matchを発火したかどうか(bool)
        """
        if not key_name or not NEXT_MATCH_KEY_NAME:
            return False
        if key_name.lower() != NEXT_MATCH_KEY_NAME.lower():
            return False
        now = time.time()
        if now - self._last_press_time < BUTTON_DEBOUNCE_SEC:
            return False  # チャタリング防止
        self._last_press_time = now
        try:
            self.on_next_match()
        except Exception as e:
            print(f"[NextMatchKeyListener] コールバック実行中にエラー: {e}")
        return True
    ...
    def start(self):
        ...
        def _on_press(key):
            self.handle_key_event(_key_to_name(key))
